...
points = []  # 都市のリスト
new_list = np.array([]) #きれいに表示するためのリスト
spot_list = da.get_place()
for i in spot_list:
    new_list = np.append(new_list, i)
points = new_list.reshape(5, 2).astype(np.float)
print(new_list)
print(points)

# points = new_list[:, 1:].astype(np.float)

# 初期集団を生成(各都市を一巡する経路の集団)q
population = []  # 集団
